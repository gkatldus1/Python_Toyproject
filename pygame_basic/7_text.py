...
# 적 enemy 캐릭터   
enemy = pygame.image.load("/Users/hamsiyeon/Desktop/PythonWorkspace/pygame_basic/enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해옴
enemy_width = enemy_size[0] #캐릭터의 가로크기
enemy_height = enemy_size[1] # 캐럭터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)# 화면 가로의 절반에 위치
enemy_y_pos = (screen_height / 2)- (enemy_height /2) # 화면의 세로 중간에 위치
    ...
    for event in pygame.event.get(): #어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌러졌는지 확인
            if event.key == pygame.K_LEFT: # 캐릭터 왼쪽으로 이동
                ...
                to_x += character_speed
            elif event.key == pygame.K_DOWN: # 캐릭터가 아래로 이동
                to_y += character_speed
            elif event.key == pygame.K_UP: # 캐릭터가 위로 이동    
                to_y -= character_speed

        if event.type == pygame.KEYUP: # 방향키를 떼면 멈충
